...
func (dao Order) Insert(ctx context.Context, userID int64, orderNumber string) error {
	_, err := dao.DB.Exec(ctx, "insert into user_orders(order_number, user_id, uploaded_at, status) values($1, $2, $3, $4)", orderNumber, userID, time.Now(), entity.OrderStatusNew)
	if err != nil {
		return err
	}
	return nil
...
func (dao Order) GetByUserID(ctx context.Context, userID int64) ([]entity.Order, error) {
	rows, err := dao.DB.Query(ctx, "SELECT order_number, user_id, uploaded_at, status, accrual_amount FROM user_orders WHERE user_id = $1 ORDER BY uploaded_at", userID)
	if err != nil {
		return nil, err
	}
...
func (dao Order) GetByOrderNumber(ctx context.Context, orderNumber string) (*entity.Order, error) {
	row := dao.DB.QueryRow(ctx, "SELECT order_number, user_id, uploaded_at, status, accrual_amount FROM user_orders WHERE order_number = $1", orderNumber)
